...
async function carregarTrilhas() {
  ...
  if (!catalogo.length) {
    list.innerHTML = `<p style="color:#6b7aaa;text-align:center;padding:40px;grid-column:1/-1">
  ...
  catalogo.forEach((m, i) => {
    ...
    card.innerHTML = `
      <div class="trilha-top">
        ...
        <div class="trilha-pct-badge" style="background:${cor};box-shadow:0 3px 12px rgba(0,0,0,0.22);border:2px solid rgba(255,255,255,0.6);">
...
async function selecionarMateria(m, prog, cardEl) {
  ...
  // Verifica se não houve outra seleção enquanto aguardava as promises
  ...
  topicos.forEach((t, i) => {
    ...
    item.innerHTML = `
      <div class="topico-left">
        <div class="topico-status ${done ? "done" : "ready"}">${done ? "✓" : "📖"}</div>
        <div>
          <div class="topico-title">${t}</div>
          <div class="topico-sub">${done ? "Concluída" : "Disponível"}</div>
        </div>
      </div>
      <div class="topico-right">
        <span class="topico-xp">+${(i + 1) * 20} XP</span>
        <button class="btn-iniciar ${done ? "repetir" : ""}" 
                style="${done ? `background:${cor};color:#fff;border:none;` : ''}">
